...
nCores <- detectCores() - 1  # Usar uno menos que el número total de núcleos
...
#Entreno en 1 año y medio quitando meses malos de la pandemia.
...
semillero <- c( 391553, 142979, 607727 , 115979, 181001, 
                186227, 191057, 298451, 449557, 457981
              )
#------------------------------------------------------------------------------
...
PARAM$bo_lgb <- makeParamSet(
  makeNumericParam("learning_rate", lower = 0.02, upper = 1),
...
modelos <- foreach(semilla = semillero, .packages = c("data.table", "rlist", "lightgbm", "DiceKriging", "mlrMBO"), .combine = 'c') %dopar% {
   PARAM$lgb_basicos$seed <- semilla
   set.seed(semilla, kind = "L'Ecuyer-CMRG")
   ...
   return(list(as.character(semilla) <- modelo_train))
}
