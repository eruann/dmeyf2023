...
# Check for saved data files and load the last one if found
saved_files = list.files(pattern="result_\d+.csv")
if (length(saved_files) > 0) {
  last_saved_file = saved_files[which.max(as.numeric(gsub("result_(\d+).csv", "\1", saved_files)))]
  result_dataset = fread(last_saved_file)
  start_client = as.numeric(gsub("result_(\d+).csv", "\1", last_saved_file)) + 1
} else {
  result_dataset = data.table()  # Initialize an empty result_dataset if no saved files found
  start_client = 1
}

  tryCatch({
    
    # ... Existing code to process the client ...
    ...
    fwrite(result_dataset, paste0("partial_result_", cliente, ".csv"))
  })

  # Save the result_dataset periodically (e.g., every 1000 clients)
  if (cliente %% 1000 == 0) {
    fwrite(result_dataset, paste0("result_", cliente, ".csv"))
  }
