...
# Configuramos el número de núcleos para paralelizarplan(multiprocess)# ... [resto del código anterior]# Función modificada para procesar a los clientes en paralelocompute_lags_velocity_acceleration_parallel <- function(dataset, features) {    # ... [resto de la función sin cambios]  # Usamos future_map_dfr para procesar a los clientes en paralelo  result_dataset <- dataset %>%                      group_split(numero_de_cliente) %>%                      future_map_dfr(~compute_lags_velocity_acceleration(.x, features))    return(result_dataset)}# Call the compute_lags_velocity_acceleration_parallel functiondataset <- dataset[order(dataset$numero_de_cliente, dataset$foto_mes), ]dataset <- compute_lags_velocity_acceleration_parallel(dataset, setdiff(names(dataset), 'clase_ternaria'))dataset <- dataset[order(dataset$numero_de_cliente, dataset$foto_mes), ]saveRDS(dataset, dataset_path)file.create(flag_file_path)# ahora SI comienza la optimizacion Bayesiana
