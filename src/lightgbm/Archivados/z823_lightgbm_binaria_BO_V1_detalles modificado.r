...
# Function to process data for a specific client
process_client_data <- function(cliente) {
  dataset <- dataset_global
  features <- setdiff(names(dataset), 'clase_ternaria')
  
  # Original code for processing client data here...
  # ... [rest of the code to process client data]
...
if (!file.exists(dataset_path) || !file.exists(flag_file_path)) {
  ...
  # Sort the dataset by client and month
  ...
  # Use parLapply to process data for each client in parallel
  num_cores <- detectCores() - 1  # Use all cores except one
  cl <- makeCluster(num_cores)
  
  # Make dataset globally available to all nodes
  dataset_global <- dataset
  
  processed_data_list <- parLapply(cl, unique_clientes, process_client_data)
