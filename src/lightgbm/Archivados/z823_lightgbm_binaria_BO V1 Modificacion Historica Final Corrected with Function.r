...
compute_lags_velocity_acceleration <- function(dataset, features) {
  # Exclude the "numero_de_cliente" column from the features for lag calculations
  features <- setdiff(features, "numero_de_cliente")
  ...
  for (cliente in unique_clientes) {
    ...
    result_dataset <- rbind(result_dataset, cliente_data)
  }
