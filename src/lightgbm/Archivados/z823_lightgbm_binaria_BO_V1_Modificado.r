...
# Función para procesar datos de un cliente específico
process_client_data <- function(cliente, dataset, features) {
  cliente_data <- dataset[dataset$numero_de_cliente == cliente, ]
  # ... [resto del código para procesar datos del cliente]
  return(cliente_data)
}

# ... [resto del código previo]

# Si el dataset no ha sido procesado previamente
if (!file.exists(dataset_path) || !file.exists(flag_file_path)) {

  # Ordenar el conjunto de datos por cliente y mes
  ...
  # Usar parLapply para procesar datos de cada cliente en paralelo
  num_cores <- detectCores() - 1  # Usar todos los núcleos excepto uno
  cl <- makeCluster(num_cores)
  
  processed_data_list <- parLapply(cl, unique_clientes, process_client_data, dataset = dataset, features = setdiff(names(dataset), 'clase_ternaria'))
  
  # Detener el cluster
  stopCluster(cl)
  
  # Combinar datos procesados
  dataset_processed <- rbindlist(processed_data_list, fill = TRUE)
  
  # Guardar el dataset procesado
  saveRDS(dataset_processed, dataset_path)
