...
PARAM$input$dataset <- "./datasets/competencia_02.csv.gz_P"

# File paths
dataset_path <- "~/buckets/b1/datasets/processed_dataset_lag_velocity_acceleration.rds_P"
flag_file_path <- "~/buckets/b1/datasets/completed_lag_velocity_acceleration.txt_P"
...
compute_lags_velocity_acceleration_parallel <- function(dataset, features) {
  ...
  clusterExport(cl, c("get_valid_lags", "dataset", "features"))
...
# Filter the dataset for the current cliente
  ...
  result_dataset <- rbindlist(list_of_results)
