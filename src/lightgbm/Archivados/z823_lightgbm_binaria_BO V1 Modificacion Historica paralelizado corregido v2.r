...
PARAM$experimento <- "HT8230_Historic_P"

PARAM$input$dataset <- "./datasets/competencia_02.csv.gz"

# File paths
dataset_path <- "~/buckets/b1/datasets/processed_dataset_lag_velocity_acceleration_P.rds"
flag_file_path <- "~/buckets/b1/datasets/completed_lag_velocity_acceleration_P.txt"
...
get_valid_lags <- function(current_month, all_months) {
  potential_lags <- c()
  for (i in 1:6) {
    last_month <- as.numeric(substr(current_month, 5, 6)) - i
    last_year <- as.numeric(substr(current_month, 1, 4))
    if (last_month <= 0) {
      last_month <- 12 + last_month
      last_year <- last_year - 1
    }
    potential_month <- paste0(last_year, sprintf("%02d", last_month))
    if (potential_month %in% all_months) {
      potential_lags <- c(potential_lags, potential_month)
    }
  }
  return(potential_lags)
}
...
compute_lags_velocity_acceleration_parallel <- function(dataset, features) {
  # Exclude the "numero_de_cliente" and "foto_mes" columns from the features for lag calculations
  features_to_use <- setdiff(features, c("numero_de_cliente", "foto_mes"))
  ...
  # Export the necessary functions and objects to the workers
  clusterExport(cl, c("get_valid_lags", "dataset", "features_to_use", "setdiff", "warning"))
  
  # Use parLapply to process each cliente in parallel
  list_of_results <- parLapply(cl, unique_clientes, function(cliente) {
    # Load the necessary library inside the parallel workers
    library(data.table)
    
    
  # Write progress to a file
  progress_file <- "features.txt"
  cat(paste0("Processed cliente: ", cliente, " (", which(unique_clientes == cliente), "/", total_clientes, ")
"), file = progress_file, append = TRUE)

# Filter the dataset for the current cliente
  ...
  # Concatenate all cliente results into a single dataset
  resulting_dataset <- rbindlist(list_of_results)
  
  # Ensure all original columns are present
  all_columns <- union(names(dataset), names(resulting_dataset))
  for (col in all_columns) {
    if (!(col %in% names(resulting_dataset))) {
      resulting_dataset[, (col) := NA]
    }
  }
  
  # Order the columns to match the original dataset with the new features appended
  resulting_dataset <- resulting_dataset[, ..all_columns]
  
  return(resulting_dataset)
}
stopCluster(cl)
  
  # Combine the results from each cliente into a single data.table
  result_dataset <- rbindlist(list_of_results, fill=TRUE)
  
  return(result_dataset)
}
# Call the compute_lags_velocity_acceleration function
dataset <- dataset[order(dataset$numero_de_cliente, dataset$foto_mes), ]
dataset <- compute_lags_velocity_acceleration_parallel(dataset, setdiff(names(dataset), 'clase_ternaria'))
dataset <- dataset[order(dataset$numero_de_cliente, dataset$foto_mes), ]
