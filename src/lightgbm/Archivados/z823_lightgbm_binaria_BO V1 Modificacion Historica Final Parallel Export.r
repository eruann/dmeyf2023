...
compute_lags_velocity_acceleration_parallel <- function(dataset, features) {
  ...
  # Register the parallel backend using all available cores
  cl <- parallel::makeCluster(detectCores())
  doParallel::registerDoParallel(cl)
  
  # Export the necessary functions and objects to the workers
  parallel::clusterExport(cl, c("get_valid_lags"))
  
  # Use foreach to process each cliente in parallel and combine the results using rbindlist
  result_dataset <- foreach(cliente = unique_clientes, .combine = data.table::rbindlist, .packages = c("data.table")) %dopar% {
