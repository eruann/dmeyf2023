...
compute_lags_velocity_acceleration <- function(dataset, features) {
  # List of all unique months in the dataset
  all_months <- unique(dataset$foto_mes)
  
  total_features <- length(features)
  total_months <- length(all_months)

  # For each feature, compute the lags, velocity, and acceleration
  for (f_index in 1:total_features) {
    feature <- features[f_index]
    cat(paste("\nProcessing feature", f_index, "of", total_features, ":", feature, "\n"))
    
    for (m_index in 1:total_months) {
      month <- all_months[m_index]
      #cat(paste("  Processing month", m_index, "of", total_months, ":", month, "\n"))
      
      valid_lags <- get_valid_lags(month, all_months)
      for (i in 1:length(valid_lags)) {
        # Compute Lag
        lag_col_name <- paste0(feature, "_lag", i)
        current_month_indices <- dataset$foto_mes == month
        previous_month_values <- dataset[dataset$foto_mes == valid_lags[i], ..feature, with = FALSE]
        
        if (sum(current_month_indices) == length(previous_month_values)) {
          dataset[current_month_indices, (lag_col_name) := previous_month_values]
        } else {
        ...
        dataset[current_month_indices, (velocity_col_name) := .SD[[feature]] - .SD[[lag_col_name]], .SDcols = c(feature, lag_col_name)]
        ...
        if (i > 1) {
          ...
          dataset[current_month_indices, (acceleration_col_name) := .SD[[velocity_col_name]] - .SD[[previous_velocity_col_name]], .SDcols = c(velocity_col_name, previous_velocity_col_name)]
        }
      }
    }
  }
